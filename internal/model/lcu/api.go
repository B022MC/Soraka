...
type (
	...
	GameInfo struct {
		...
		GameMode              lol.GameMode `json:"gameMode"`
		GameType              lol.GameType `json:"gameType"`
		...
		Participants []struct { // 参与者详细信息
			ChampionId                lol.Champion `json:"championId"` // 英雄id
			HighestAchievedSeasonTier string       `json:"highestAchievedSeasonTier"`
			ParticipantId             int          `json:"participantId"`
			Spell1Id                  lol.Spell    `json:"spell1Id"` // 召唤师技能1
			Spell2Id                  lol.Spell    `json:"spell2Id"` // 召唤师技能2
			Stats                     struct {
		...
		PlatformId string          `json:"platformId"` // 平台id
		QueueId    lol.GameQueueID `json:"queueId"`    // 队列id
	...
	Participant struct {
		...
		Timeline struct {
			...
			Lane               lol.Lane         `json:"lane"`
			ParticipantId      int              `json:"participantId"`
			Role               lol.ChampionRole `json:"role"`
	...
	GameFolwSessionTeamUser struct {
		...
		TeamId                  lol.TeamIDStr `json:"teamId,omitempty"`
	}
...
type RankedStatsResponse struct {
	...
	SplitsProgress                    map[string]interface{} `json:"splitsProgress"` // 可根据需要定义更细
}
